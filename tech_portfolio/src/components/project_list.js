const project_list = [
  {
    id: 10,
    title: "Construction Project Management Platform",
    description:
      "Built a secure construction management platform around the work crews already do each day. Teams can sign in, organize projects and tasks, attach jobsite images, and generate estimates with AI. The project record and the work surrounding it stay together in one place.",
    tech: ["React", "JavaScript", "Python", "PostgreSQL", "OpenAI"],
  },
  {
    id: 2,
    title: "RealEstate SMS Program",
    description:
      "A customizable SMS-based platform for real estate professionals that enables clients to create, edit, and manage automated question flows. The system sends predefined questions to users via text and handles responses intelligently using AI-driven auto-replies.",
    tech: [
      "React",
      "JavaScript",
      "Flask",
      "TailwindCSS",
      "Python",
      "OpenAI",
      "AWS",
      "PostgreSQL",
      "Twilio",
    ],
  },
  {
    id: 3,
    title: "Automated Messaging System for Daycare",
    description:
      "An intelligent messaging tool designed for daycares that sends mass communications to parents and uses AI to analyze incoming replies for sentiment and relevance. It helps staff quickly identify concerns, confirmations, or follow-up needs based on the tone and content of each response.",
    tech: [
      "OpenAI",
      "Gmail",
      "Python",
      "Google Sheets",
      "PostgreSQL",
      "Twilio",
    ],
  },
  {
    id: 1,
    title: "Call Transcript Event Generator",
    description:
      "A smart React-based application that transcribes phone calls in real time and automatically schedules events based on the conversation. For example, if a meeting is discussed for June 5th during the call, the app detects the context and creates a calendar event with all the relevant details.",
    tech: [
      "React",
      "Python",
      "Flask",
      "JavaScript",
      "TailwindCSS",
      "OpenAI",
      "Azure",
      "PostgreSQL",
      "Google Calendar",
    ],
  },
];

export default project_list;
